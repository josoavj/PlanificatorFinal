...
Ce document détaille les mesures de sécurité implémentées dans le projet Planificator pour protéger les données des clients et l'intégrité du système.

## 🔒 1. Protection des Identifiants (Credentials)

L'application ne stocke jamais les identifiants de la base de données en texte clair.
...
- **Principe du Moindre Privilège** : Il est recommandé de créer un utilisateur MySQL spécifique pour l'application avec uniquement les droits `SELECT`, `INSERT`, `UPDATE`, `DELETE` sur la base `Planificator`.
...
Bien que l'application soit renforcée, les mesures suivantes sont impératives pour un déploiement réel :
...
3. **Mises à jour** : Maintenez régulièrement les dépendances de l'application via `flutter pub upgrade` pour corriger les éventuelles failles découvertes dans les bibliothèques tierces.
